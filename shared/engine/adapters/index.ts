...
// Auto-register all generic/fallback adapters
// ORDER MATTERS: adapters are checked in import order when match scores are equal.
// GenericDirectoryParser (0.05) must come BEFORE UniversalAIParser (0.02) so
...
// Lazy load specific adapters based on URL patterns
parserRegistry.registerLazy({
  id: 'saudi-chamber',
  name: 'Saudi Chamber',
  pattern: /chamber\.sa|mcci\.org\.sa|chamber\.org\.sa|fsc\.org\.sa|\/members/i,
  loader: () => import('./SaudiChamberAdapter')
});

parserRegistry.registerLazy({
  id: 'b2b-portal',
  name: 'B2B Portal',
  pattern: /alibaba\.com|kompass\.com|\/suppliers\//i,
  loader: () => import('./B2BPortalAdapter')
});

parserRegistry.registerLazy({
  id: 'saudi-gov-registry',
  name: 'Saudi Gov Registry',
  pattern: /\.gov\.sa|\/cr|\/license/i,
  loader: () => import('./SaudiGovRegistryAdapter')
});

parserRegistry.registerLazy({
  id: 'modon',
  name: 'Modon',
  pattern: /modon\.gov\.sa/i,
  loader: () => import('./ModonAdapter')
});

parserRegistry.registerLazy({
  id: 'saudi-industry-guide',
  name: 'Saudi Industry Guide',
  pattern: /saudiindustryguide\.com/i,
  loader: () => import('./SaudiIndustryGuideAdapter')
});
...
parserRegistry.registerLazy({
  ...
  name: 'Cybo',
  pattern: /cybo\.com/i,
  loader: () => import('./CyboAdapter')
});

parserRegistry.registerLazy({
  id: 'bizmideast',
  name: 'Biz Mideast',
  pattern: /bizmideast\.com/i,
  loader: () => import('./BizMideastAdapter')
});

parserRegistry.registerLazy({
  id: 'ksa-directory',
  name: 'KSA Directory',
  pattern: /ksa\.directory|saudidir\.com/i,
  loader: () => import('./KSADirectoryAdapter')
});

import './UniversalAIParser';             // AI fallback — LAST resort (0.02)
 ...
 *
 * 1. Create a new file: `src/engine/adapters/YourAdapter.ts`
